...
from model_base123 import embed_net
from utils import *
from loss import OriTripletLoss, TriLoss, DCLoss, CenterTripletLoss, KLDivLoss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta', default=0.3, type=float, metavar='delta',
                        help='dcl weights, 0.2 for PCB, 0.5 for resnet50')
    ...
    def train(epoch):
        ...
        for batch_idx, (input1, input2, label1, label2) in enumerate(trainloader):
            ...
            feat1, feat2, feat3, feat4, out1, out2, out3, out4,x11,x21,outx11,outx21 = net(input1, input2)
            lbs = torch.cat((label1, label2), 0)
            # print(out1.size())
            loss_id = (criterion_id(out1, labels.long()) + criterion_id(out2, labels.long()) + criterion_id(out3,
                                                                                                            labels.long()) + criterion_id(
                out4, labels.long())) * 0.25

            loss_kl1 = criterion_kl(outx11, outx21)
            # loss_tri = (criterion_tri(feat1,labels)[0]+criterion_tri(feat2,labels)[0]+criterion_tri(feat3,labels)[0]
            # +criterion_tri(feat4,labels)[0])/4
            # outx111,outx1111 = outx11.chunk(2,0)
            # outx222,outx2222 = outx21.chunk(2,0)
            # loss_kl1 = (criterion_kl(outx1111, outx111)+criterion_kl(outx2222,outx222))/2\
            # loss_kl1 = criterion_kl(outx11, outx21)
            # ft11 ,ft13= torch.chunk(feat1, 2, 0)
            # ft21, ft23= torch.chunk(feat2, 2, 0)
            # ft31, ft33 = torch.chunk(feat3, 2, 0)
            # ft41, ft43 = torch.chunk(feat4, 2, 0)
            #
            # ft12 = ft11
            # ft14 = ft13
            # ft22 = ft21
            ...
            ba = criterion_tri(feat1, labels)[1]
            # loss_tri5 = (criterion_tri(x11, labels)[0] + criterion_tri(x21, labels)[0]) / 2
            loss_tri1 = (criterion_tri(torch.cat((ft11, ft13), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft11, ft12), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft13, ft14), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft11, ft14), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft12, ft13), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft12, ft14), 0), lbs)[0]) / 6
            loss_tri2 = (criterion_tri(torch.cat((ft21, ft23), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft21, ft22), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft23, ft24), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft21, ft24), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft22, ft23), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft22, ft24), 0), lbs)[0]) / 6
            loss_tri3 = (criterion_tri(torch.cat((ft31, ft33), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft31, ft34), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft31, ft32), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft33, ft34), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft32, ft33), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft32, ft34), 0), lbs)[0]) / 6
            loss_tri4 = (criterion_tri(torch.cat((ft41, ft43), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft41, ft44), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft41, ft42), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft43, ft44), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft42, ft43), 0), lbs)[0] +
                         criterion_tri(torch.cat((ft42, ft44), 0), lbs)[0]) / 6

            loss_tri = loss_tri1 + loss_tri2 +loss_tri3+loss_tri4

            # loss_dcl = (criterion_div(torch.cat((ft12, ft14), 0)) + criterion_div(
            # 	torch.cat((ft22, ft24), 0)) + criterion_div(torch.cat((ft32, ft34), 0)) + criterion_div(
            # 	torch.cat((ft42, ft44), 0))) * 0.25 * args.delta
            # loss_lmd = (criterion_lmd(ft11,ft13,ft12,ft14,labels)+criterion_lmd(ft21,ft23,ft22,ft24,labels)+criterion_lmd(ft31,ft33,ft32,ft34,labels)+criterion_lmd(ft41,ft43,ft42,ft44,labels))*0.25*args.delta

            correct += (ba / 2)
            _, predicted = out1.max(1)
            correct += (predicted.eq(labels).sum().item() / 2)

            loss = loss_id + loss_tri+ args.delta * loss_kl1
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
