...
from model_base1231 import embed_net
from utils import *
import pdb
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_path', default='/root/autodl-tmp/save_modelbase1+2+3(1)/', type=str,
                        help='model save path')
    parser.add_argument('--save_epoch', default=20, type=int,
                        metavar='s', help='save model every 10 epochs')
    parser.add_argument('--log_path', default='/root/autodl-tmp/logbase1+2+3(1)/', type=str,
                        help='log save path')
    parser.add_argument('--vis_log_path', default='/root/autodl-tmp/logbase1+2+3(1)/vis_log/', type=str,
                        help='log save path')
    ...
    parser.add_argument('--re_rank', default='random_walk', type=str,
    ...
    def extract_gall_feat(gall_loader):
        ...
        if args.pcb == 'on':
            return gall_feat_att,gall_feat
        else:
    ...
    def extract_query_feat(query_loader):
        ...
        print('Extracting Time:\t {:.3f}'.format(time.time() - start))
        if args.pcb == 'on':
            return query_feat_att,query_feat
        else:
    ...
    if dataset == 'sysu':
        ...
        if args.pcb == 'on':
            query_feat_pool,query_feat = extract_query_feat(query_loader)
        else:
        ...
        for trial in range(10):
            ...
            if args.pcb == 'on':
                gall_feat_pool,gall_feat = extract_gall_feat(trial_gall_loader)
            else:
        ...
        for trial in range(10):
            ...
            model_path = checkpoint_path + 'regdb_agw_p4_n8_lr_0.1_seed_0_trial_{}_best.t'.format(
                test_trial)
            if os.path.isfile(model_path):
                print('==> loading checkpoint {}'.format(args.resume))
                checkpoint = torch.load(model_path)
            ...
            if args.pcb == 'on':
                query_feat_pool,query_feat = extract_query_feat(query_loader)
                gall_feat_pool,gall_feat = extract_gall_feat(gall_loader)
            else:
                query_feat_pool, query_feat_fc = extract_query_feat(query_loader)
